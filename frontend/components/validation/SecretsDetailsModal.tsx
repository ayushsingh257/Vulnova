...
import { SecretCategoryResultDTO } from "@/services/secrets_validation.service";
import { X, Wrench, AlertTriangle, KeyRound } from "lucide-react";
import { Badge } from "@/components/ui/badge";

interface SecretsDetailsModalProps {
  category: SecretCategoryResultDTO | null;
  onClose: () => void;
}

export const SecretsDetailsModal: React.FC<SecretsDetailsModalProps> = ({
  category,
  onClose,
}) => {
        ...
        <div className="space-y-6">
          ...
          {category.failure_reason && (
            <div className="p-4 rounded-lg bg-amber-950/30 border border-amber-800/50 space-y-1">
              <div className="flex items-center space-x-2 font-bold text-amber-300 text-xs">
                ...
                <span>Secrets & Cryptographic Diagnostic</span>
              </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 text-xs font-bold text-white uppercase tracking-wider">
              ...
              <span>Recommended Cryptographic Remediation</span>
            </div>
