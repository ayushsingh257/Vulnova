...
import { X, ShieldAlert, CheckCircle2, Wrench, FileText, AlertTriangle } from "lucide-react";
import { Badge } from "@/components/ui/badge";

interface OWASPTestDetailsModalProps {
  category: OWASPCategoryResultDTO | null;
  onClose: () => void;
}

export const OWASPTestDetailsModal: React.FC<OWASPTestDetailsModalProps> = ({
  category,
  onClose,
}) => {
        ...
        <div className="space-y-6">
          ...
          {category.failure_reason && (
            <div className="p-4 rounded-lg bg-amber-950/30 border border-amber-800/50 space-y-1">
              <div className="flex items-center space-x-2 font-bold text-amber-300 text-xs">
                ...
                <span>Assertion Failure Diagnostic</span>
              </div>
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-bold text-zinc-400 block uppercase tracking-wider">
              Affected Subsystem / Architecture Layer
            </span>
            <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800 text-xs font-mono text-purple-300">
              {category.affected_subsystem || "Core Platform Controls"}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800">
              ...
              <span className="text-[11px] text-zinc-400">Total Assertions</span>
            </div>
            <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800">
          ...
          {/* Affected Findings */}
          {category.affected_finding_ids.length > 0 && (
            <div className="space-y-2">
              <span className="text-xs font-bold text-zinc-400 block uppercase tracking-wider">
                Related Security Findings ({category.affected_finding_ids.length})
              </span>
              <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                {category.affected_finding_ids.map((id) => (
                  <div key={id} className="p-2 rounded bg-zinc-900 border border-zinc-800 text-xs font-mono text-zinc-300">
                    Finding ID: {id}
                  </div>
                ))}
              </div>
