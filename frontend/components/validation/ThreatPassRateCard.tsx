...
interface ThreatPassRateCardProps {
  passRate: number;
...
export const ThreatPassRateCard: React.FC<ThreatPassRateCardProps> = ({
  passRate,
  overallStatus,
  passedCount,
  failedCount,
    ...
    <Card className="border-zinc-800 bg-zinc-950/80 backdrop-blur-md">
      <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-zinc-800">
        <div className="flex items-center space-x-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-600/20 border border-orange-500/40 text-orange-400">
            <ShieldCheck className="h-5 w-5" />
          </div>
          <div>
            <CardTitle className="text-base font-bold text-white">
              Threat Model & STRIDE Verification Security Posture
            </CardTitle>
            <p className="text-xs text-zinc-400 mt-0.5">
              Automated Spoofing, Tampering, Repudiation, Info Disclosure, DoS & Privilege Escalation audit score
            </p>
          </div>
        </div>
      ...
      <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
        ...
        <div className="flex flex-col items-center justify-center border-r border-zinc-800/80 pr-4">
          ...
          <span className="text-xs text-zinc-400">STRIDE Controls Passed</span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-zinc-400">STRIDE Violations</span>
        </div>
      </CardContent>
    </Card>
  );
