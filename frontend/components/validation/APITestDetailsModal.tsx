...
import { APIValidationCategoryResultDTO } from "@/services/api_security_validation.service";
import { X, Wrench, AlertTriangle, Link2, Shield } from "lucide-react";
import { Badge } from "@/components/ui/badge";

interface APITestDetailsModalProps {
  category: APIValidationCategoryResultDTO | null;
  onClose: () => void;
}

export const APITestDetailsModal: React.FC<APITestDetailsModalProps> = ({
  category,
  onClose,
}) => {
        ...
        <div className="space-y-6">
          ...
          {category.failure_reason && (
            <div className="p-4 rounded-lg bg-amber-950/30 border border-amber-800/50 space-y-1">
              <div className="flex items-center space-x-2 font-bold text-amber-300 text-xs">
                ...
                <span>API Assertion Diagnostic</span>
              </div>
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-bold text-zinc-400 flex items-center space-x-1 uppercase tracking-wider">
              ...
              <span>Sample Affected API Route</span>
            </span>
            <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800 text-xs font-mono text-blue-300">
              {category.affected_endpoint || "Global REST Endpoint Scope"}
            </div>
          </div>
