...
interface APIValidationCategoryGridProps {
  categories: APIValidationCategoryResultDTO[];
  onSelectCategory: (cat: APIValidationCategoryResultDTO) => void;
}

export const APIValidationCategoryGrid: React.FC<APIValidationCategoryGridProps> = ({
  categories,
  onSelectCategory,
}) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {categories.map((cat) => (
        ...
        >
          ...
          {/* Endpoint & Subsystem Info */}
          <div className="flex items-center justify-between text-xs text-zinc-400">
            <span className="font-mono text-[11px] truncate max-w-[200px]" title={cat.affected_endpoint || "Global REST Ingress"}>
              Route: {cat.affected_endpoint || "Global REST Ingress"}
            </span>
            <span className="font-mono text-[11px] text-zinc-400 truncate max-w-[150px]">
              {cat.affected_subsystem || "Core API Control"}
            </span>
          </div>
