...
│   │   │   ├── ai/                  # LLMGatewayService (fallback, health cooldown, cost estimation), PromptOrchestratorService (context builder, secret masking), AIFindingExplainerService (JSON repair recovery), ImpactAnalysisService (CVSS/EPSS/topology context enrichment), DTOs
...
│   │   │   ├── entities/            # User, Organization, Target, Finding, CVSS, EPSS, Evidence, ScanProfile, ScanPolicy, AssetSnapshot, AssetChangeEvent, TriageRecord, LLMProvider, LLMModel, PromptTemplate, AIFindingExplanation, AIImpactAnalysis entities
...
│   │   │   ├── db/                  # Async SQLAlchemy models (AssetSnapshotModel, LLMProviderModel, LLMModelRegistryModel, PromptTemplateModel, LLMRequestLogModel, AIFindingExplanationModel, AIImpactAnalysisModel), Alembic migrations, Repositories (LLMGatewayRepository, FindingTriageRepository, AIAnalysisRepository)
...
│   └── tests/                       # Pytest unit, integration, and security test suite (192 passed)
│
