...
import { Boxes, AlertTriangle, XCircle, CheckCircle2 } from "lucide-react";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

interface ContainerPassRateCardProps {
  passRate: number;
...
export const ContainerPassRateCard: React.FC<ContainerPassRateCardProps> = ({
  passRate,
  overallStatus,
  passedCount,
  failedCount,
    ...
    <Card className="border-zinc-800 bg-zinc-950/80 backdrop-blur-md">
      <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-zinc-800">
        <div className="flex items-center space-x-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-600/20 border border-cyan-500/40 text-cyan-400">
            <Boxes className="h-5 w-5" />
          </div>
          <div>
            <CardTitle className="text-base font-bold text-white">
              Container Image Security & Runtime Hardening Posture
            </CardTitle>
            <p className="text-xs text-zinc-400 mt-0.5">
              Automated base image CVEs, unprivileged execution, capability drops & cgroup isolation score
            </p>
          </div>
        </div>
      ...
      <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
        ...
        <div className="flex flex-col items-center justify-center border-r border-zinc-800/80 pr-4">
          ...
          <span className="text-xs text-zinc-400">Hardening Controls Passed</span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-zinc-400">Container Violations</span>
        </div>
      </CardContent>
    </Card>
  );
