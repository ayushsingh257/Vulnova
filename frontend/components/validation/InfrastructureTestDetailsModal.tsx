...
import { InfrastructureValidationCategoryResultDTO } from "@/services/infrastructure_validation.service";
import { X, Wrench, AlertTriangle, Layers, ShieldCheck } from "lucide-react";
import { Badge } from "@/components/ui/badge";

interface InfrastructureTestDetailsModalProps {
  category: InfrastructureValidationCategoryResultDTO | null;
  onClose: () => void;
}

export const InfrastructureTestDetailsModal: React.FC<InfrastructureTestDetailsModalProps> = ({
  category,
  onClose,
}) => {
        ...
        <div className="space-y-6">
          ...
          {category.failure_reason && (
            <div className="p-4 rounded-lg bg-amber-950/30 border border-amber-800/50 space-y-1">
              <div className="flex items-center space-x-2 font-bold text-amber-300 text-xs">
                ...
                <span>Infrastructure Assertion Diagnostic</span>
              </div>
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-bold text-zinc-400 flex items-center space-x-1 uppercase tracking-wider">
              ...
              <span>Evaluated Infrastructure Component</span>
            </span>
            <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800 text-xs font-mono text-emerald-300">
              {category.affected_component || "Infrastructure Layer"}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 text-xs font-bold text-white uppercase tracking-wider">
              ...
              <span>Recommended Infrastructure Remediation</span>
            </div>
