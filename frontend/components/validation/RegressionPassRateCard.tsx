...
interface RegressionPassRateCardProps {
  passRate: number;
...
export const RegressionPassRateCard: React.FC<RegressionPassRateCardProps> = ({
  passRate,
  overallStatus,
  passedCount,
  failedCount,
    ...
    <Card className="border-zinc-800 bg-zinc-950/80 backdrop-blur-md">
      <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-zinc-800">
        <div className="flex items-center space-x-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-600/20 border border-teal-500/40 text-teal-400">
            <ShieldAlert className="h-5 w-5" />
          </div>
          <div>
            <CardTitle className="text-base font-bold text-white">
              Automated Security Regression Testing Posture
            </CardTitle>
            <p className="text-xs text-zinc-400 mt-0.5">
              Continuous security regression matrix protecting OWASP Web/API, Infra, Exploits, SCA, Containers, Secrets & STRIDE
            </p>
          </div>
        </div>
      ...
      <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
        ...
        <div className="flex flex-col items-center justify-center border-r border-zinc-800/80 pr-4">
          ...
          <span className="text-xs text-zinc-400">Guards Passed</span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-zinc-400">Active Regressions</span>
        </div>
      </CardContent>
    </Card>
  );
