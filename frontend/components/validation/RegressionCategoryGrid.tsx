...
interface RegressionCategoryGridProps {
  categories: RegressionCategoryResultDTO[];
  onSelectCategory: (cat: RegressionCategoryResultDTO) => void;
}

export const RegressionCategoryGrid: React.FC<RegressionCategoryGridProps> = ({
  categories,
  onSelectCategory,
}) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {categories.map((cat) => (
        <div
          ...
          className="group relative p-4 rounded-xl border border-zinc-800 bg-zinc-950/80 hover:border-teal-500/50 hover:bg-zinc-900/60 transition-all cursor-pointer space-y-3"
        >
          ...
          {/* Component Info */}
          <div className="text-xs text-zinc-400 font-mono truncate" title={cat.affected_component || "Security Layer"}>
            Layer: {cat.affected_component || "Security Layer"}
          </div>
